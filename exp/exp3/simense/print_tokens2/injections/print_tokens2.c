...
/* CREST input func changed */
#ifdef  CREST
char fuzz_crest_arr[MYMAX]={'\0'};
...
int fuzzungetc(int character,FILE * stream){
    if(_crest_count>0){
        fuzz_crest_arr[--_crest_count]=(char)character;
        return character;
    }else{
        return EOF;
    }
...
{  char *fname;
 ...
 //  atexit(freeArray);
    ...
    print_token(tok); /* print eof signal */ crest_use();freeArray();
    exit(0);
}
...
int str_com_id;
{ 
 int* __CREST_p6 = (int*)malloc(sizeof(int)*10);*__CREST_p6 = 0;
...
token tok;
{ 
  int* __CREST_p10 = (int*)malloc(sizeof(int)*10);*__CREST_p10 = 0;
